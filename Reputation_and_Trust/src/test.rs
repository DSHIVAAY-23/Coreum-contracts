...
mod tests {
    ...
    //     let token_response: assetft::TokenResponse = from_binary(&res).unwrap();
    //     assert_eq!(token_response.symbol, "RPT");
    //     assert_eq!(token_response.subunit, "rpt");
    //     assert_eq!(token_response.precision, 6);
    // }

    // #[test]
    // fn test_query_reputation() {
    //     let mut deps = mock_dependencies();
    //     let env = mock_env();
    //     let info = mock_info("creator", &coins(1000, "earth"));
    ...
    //     let query_msg = QueryMsg::GetReputation { user: "user1".to_string() };
    //     let res = query(deps.as_ref(), env, query_msg).unwrap();

    //     let reputation: UserReputation = from_binary(&res).unwrap();
    //     assert_eq!(reputation.reputation, 10);
    // }
    ...
    //     BALANCES.save(&deps.storage, &Addr::unchecked("user1"), &Uint128::new(500)).unwrap();

    //     let query_msg = QueryMsg::GetBalance { user: "user1".to_string() };
    //     let res = query(deps.as_ref(), env, query_msg).unwrap();
